...
=================
  ...
  #. start Nginx or reload Nginx (see Nginx on premise setup guide in
...
* 2.1.3.1 location /
  ...
  - access_by_lua_file: This should be the path to the Lua configuration
    file in your server (e.g.
    /opt/openresty/nginx/conf/nginx_12345.lua).

  - proxy_pass: If the authentication performed during the
    *access_by_lua_file* step has been successful then this instruction
    will perform a pass through. The variable *$proxy_pass* will have
    been set to point to the correct upstream in the Lua code.
...
---------------
  ...
  function with the following naming pattern:
  **extract_usage_SERVICEID**.
  ...
  |     ngx.var.proxy_pass = "https://backend_myapidomain.com"
  |     ngx.var.usage = extract_usage_2555417709352(ngx.var.request)
  |  end
  `----
...
===========
  ...
  [Advanced settings for Nginx in 3scale]
  https://support.3scale.net/howtos/api-configuration#nginx-advanced
